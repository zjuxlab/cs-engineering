...
下载地址：<u>https://www.enterprisedb.com/downloads/postgres-postgresql-downloads</u>。

双击安装包，开始安装

![](/assets/KU3NbfCnBoeHFCxSGF6cOTqlnPg.png)

一路默认选项到最后，取消掉"Stack ..."的勾选，然后点击finish

![](/assets/OLFmb79Mvo0ynlxBN7mcKXhInOb.png)

这样就安装成功了，如果要使用的话，有两种方法，GUI的方式是打开pgAdmin，非GUI的话就打开SQL Shell

### Linux安装

打开 PostgreSQL 官网 <u>https://www.postgresql.org/</u>，点击菜单栏上的 <b>Download</b> ，可以看到这里包含了很多平台的安装包，包括 Linux、Windows、Mac OS等 。

Linux 我们可以看到支持 Ubuntu 和 Red Hat 等各个平台，点击具体的平台链接，即可查看安装方法：

![](/assets/TjbKbqPPZocN62xZBugc1AYknwg.jpeg)

点击上图中的 <b>file browser</b>，我们还能下载 PostgreSQL 最新的源码。

![](/assets/J8ydboHCLolRb2xJqQacZYbZnFd.jpeg)

本章节以 Ubuntu 为例。

#### <b>Ubuntu 安装 PostgreSQL</b>
...
Mac的安装流程与windows基本一致

下载地址https://www.enterprisedb.com/downloads/postgres-postgresql-downloads

相同的安装流程，如果使用GUI，则和windows一样

如果不用GUI，mac需要执行runpsql.sh脚本
...
Port [5432]: 
Username [postgres]: 
Password for user postgres:<此处可以填写安装时设置的密码> 
psql (11.3)
Type "help" for help.
...
如果打算为项目选择一款免费、开源的[数据库](https://cloud.tencent.com/solution/database?from=20065&from_column=20065)，那么你可能会在MySQL与PostgreSQL之间犹豫不定。MySQL与PostgreSQL都是免费、开源、强大、且功能丰富的数据库。你主要的问题可能是：<b>哪一个才是最好的开源数据库，MySQL还是PostgreSQL呢？该选择哪一个开源数据库呢？</b>
...
现在，基于最初的MySQL代码还有更多的数据库可供选择，因为几个核心的MySQL开发者已经发布了MySQL分支。最初的MySQL创建者之一 Michael “Monty” Widenius貌似后悔将MySQL卖给了Sun公司，于是又开发了他自己的MySQL分支[MariaDB](https://cloud.tencent.com/product/tdsql?from=20065&from_column=20065)，它是免费的，基于GPL许可。知名的 MySQL开发者Brian Aker所创建的分支Drizzle对其进行了大量的改写，特别针对多CPU、云、网络应用与高并发进行了优化。
...
支持使用非标准的自定义功能的人们认为这样可以快速采用新的特性，而不必等待标准进程完成。ANSI/ISO标准在不断演化，因此标准兼容性也是个 变化的目标：知名的关系型数据库Microsoft [SQL Server](https://cloud.tencent.com/product/sqlserver?from=20065&from_column=20065)、Oracle与IBM DB2也只是部分兼容于标准。
